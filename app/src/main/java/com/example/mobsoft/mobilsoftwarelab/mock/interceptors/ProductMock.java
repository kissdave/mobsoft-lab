...
public class ProductMock {
    public static Response process(Request request) {
        ...
        if (uri.getPath().equals(NetworkConfig.ENDPOINT_PREFIX + "Products") && request.method().equals("POST")) {
            responseString = "";
            responseCode = 200;
        } else if (uri.getPath().equals(NetworkConfig.ENDPOINT_PREFIX + "Products") && request.method().equals("Get")) {
            MemoryRepository memoryRepository = new MemoryRepository();
            memoryRepository.open(null);
            responseString = GsonHelper.getGson().toJson(memoryRepository.getProducts());
            responseCode = 200;
        } else {
