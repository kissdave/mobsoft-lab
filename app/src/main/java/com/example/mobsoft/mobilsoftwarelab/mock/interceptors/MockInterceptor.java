...
public class MockInterceptor implements Interceptor {
    ...
    public Response process(Request request) {
        ...
        if(uri.getPath().startsWith(NetworkConfig.ENDPOINT_PREFIX + "Order")) {
            return OrderMock.process(request);
        }

        if(uri.getPath().startsWith(NetworkConfig.ENDPOINT_PREFIX + "Products")) {
            return ProductMock.process(request);
        }
